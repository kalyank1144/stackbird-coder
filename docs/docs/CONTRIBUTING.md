...
Welcome! This guide provides all the details you need to contribute effectively to the project. Thank you for helping us make **stackbird.new** a better tool for developers worldwide. 💡
...
- Clone the repository:
  ```bash
  git clone https://github.com/stackblitz-labs/stackbird.new.git
  ```
...
```bash
docker run -p 5173:5173 --env-file .env.local stackbird-ai:development
```
...
```bash
docker build . --target stackbird-ai-production
```
...
```bash
docker run -p 5173:5173 --env-file .env.local stackbird-ai:production
```
