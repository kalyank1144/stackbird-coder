...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      stackbirdVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.stackbirdVersion) {
      body += `- stackbird.new: ${data.environmentInfo.stackbirdVersion}\n`;
    }
  ...
  body += '---\n*Submitted via stackbird.new bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/stackbird.new';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'stackbird.new-bug-reporter',
    });
