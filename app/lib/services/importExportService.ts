...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          stackbird_user_profile: this._safeGetItem('stackbird_user_profile'),
          stackbird_settings: this._safeGetItem('stackbird_settings'),
          stackbird_profile: this._safeGetItem('stackbird_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('stackbird_viewed_features'),
          developer_mode: this._safeGetItem('stackbird_developer_mode'),
        ...
        ui: {
          // Tab configuration
          stackbird_tab_configuration: this._safeGetItem('stackbird_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('stackbird_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('stackbird_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          stackbird_read_logs: this._safeGetItem('stackbird_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('stackbird_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('stackbird_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.stackbird_tab_configuration) {
        try {
          this._safeSetItem('stackbird_tab_configuration', data.ui.stackbird_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'stackbird_acknowledged_debug_issues',
        'stackbird_acknowledged_connection_issue',
        'error_logs',
        'stackbird_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('stackbird_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
