...
const ARTIFACT_TAG_OPEN = '<stackbirdArtifact';
const ARTIFACT_TAG_CLOSE = '</stackbirdArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<stackbirdAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</stackbirdAction>';
const STACKBIRD_QUICK_ACTIONS_OPEN = '<stackbird-quick-actions>';
const STACKBIRD_QUICK_ACTIONS_CLOSE = '</stackbird-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends StackbirdArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: StackbirdArtifactData;
  currentAction: StackbirdActionData;
  actionId: number;
}
    ...
    while (i < safeInput.length) {
      if (safeInput.startsWith(STACKBIRD_QUICK_ACTIONS_OPEN, i)) {
        ...
        if (actionsBlockEnd !== -1) {
          ...
          i = actionsBlockEnd + STACKBIRD_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as StackbirdAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as StackbirdAction,
              });
          ...
          if (potentialTag === ARTIFACT_TAG_OPEN) {
            ...
            if (openTagEnd !== -1) {
              ...
              const currentArtifact = {
                ...
                title: artifactTitle,
                type,
              } satisfies StackbirdArtifactData;
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__stackbirdQuickAction__"',
    'data-stackbird-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__stackbirdQuickAction__\" data-stackbird-quick-action=\"true\">${buttons.join('')}</div>`;
}
