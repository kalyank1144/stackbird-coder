...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<stackbirdArtifact') || input.includes('</stackbirdArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<stackbirdArtifact id="${artifactId}" title="${title}" type="bundled">
<stackbirdAction type="file" filePath="${filePath}">
${content}
</stackbirdAction>
</stackbirdArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<stackbirdArtifact id="${artifactId}" title="Shell Command" type="shell">
