...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyStackbirdActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyStackbirdActions(content);

      content = content.replace(/<div class=\\"__stackbirdThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`Stackbird failed to select files`);
  }
