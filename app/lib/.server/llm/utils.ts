...
export function simplifyStackbirdActions(input: string): string {
  // Using regex to match stackbirdAction tags that have type="file"
  const regex = /(<stackbirdAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/stackbirdAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<stackbirdAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</stackbirdAction>`;
    });

  return `<stackbirdArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</stackbirdArtifact>`;
}
