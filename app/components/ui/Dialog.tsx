...
export const DialogTitle = memo(({ className, children, ...props }: RadixDialog.DialogTitleProps) => {
  return (
    <RadixDialog.Title
      className={classNames('text-lg font-medium text-stackbird-elements-textPrimary flex items-center gap-2', className)}
      {...props}
    >
      {children}
    </RadixDialog.Title>
...
export const DialogDescription = memo(({ className, children, ...props }: RadixDialog.DialogDescriptionProps) => {
  return (
    <RadixDialog.Description
      className={classNames('text-sm text-stackbird-elements-textSecondary mt-1', className)}
      {...props}
    >
      {children}
    </RadixDialog.Description>
            ...
            <Button
              ...
              className={
                variant === 'destructive'
                  ...
                  : 'bg-stackbird-elements-item-backgroundAccent text-stackbird-elements-item-contentAccent hover:bg-stackbird-elements-button-primary-backgroundHover'
              }
            >
              {isLoading ? (
                <>
    ...
    return (
      <div
        ...
        className={classNames(
          ...
          selectedItems.includes(item.id)
            ? 'bg-stackbird-elements-item-backgroundAccent'
            : 'bg-stackbird-elements-bg-depth-2 hover:bg-stackbird-elements-item-backgroundActive',
        )}
        style={{
          ...style,
          width: '100%',
      ...
      >
        ...
        <div className="grid gap-1.5 leading-none">
          ...
          {item.description && <p className="text-xs text-stackbird-elements-textSecondary">{item.description}</p>}
        </div>
      </div>
    );
  };
      ...
      <Dialog showCloseButton={false}>
        <div className="p-6 bg-white dark:bg-gray-950 relative z-10">
          ...
          <DialogDescription className="mt-2 mb-4">
            ...
            <span className="text-stackbird-elements-item-contentAccent font-medium">{confirmLabel}</span>.
          </DialogDescription>

          <div className="py-4">
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-medium text-stackbird-elements-textSecondary">
              ...
              <Button
                ...
                className="text-xs h-8 px-2 text-stackbird-elements-textPrimary hover:text-stackbird-elements-item-contentAccent hover:bg-stackbird-elements-item-backgroundAccent bg-stackbird-elements-bg-depth-2 dark:bg-transparent"
              >
            ...
            <div
              className="pr-2 border rounded-md border-stackbird-elements-borderColor bg-stackbird-elements-bg-depth-2"
              style={{
                maxHeight,
              }}
            >
              {items.length > 0 ? (
                <FixedSizeList
                  ...
                  itemSize={60}
                  className="scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-stackbird-elements-bg-depth-3"
                >
                  {ItemRenderer}
                </FixedSizeList>
              ) : (
                <div className="text-center py-4 text-sm text-stackbird-elements-textTertiary">No items to display</div>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-between mt-6">
            <Button
              variant="outline"
              onClick={onClose}
              className="border-stackbird-elements-borderColor text-stackbird-elements-textPrimary hover:bg-stackbird-elements-item-backgroundActive"
            >
              Cancel
            </Button>
            <Button
